...
public class ParamTypeService {
    ...
    public ParamTypeResponse create(ParamTypeRequest paramTypeRequest) {
        ParamType paramType = paramTypeMapper.paramTypeRequestToParamType(paramTypeRequest);
    ...
    public ParamTypeResponse update(Long id, ParamTypeRequest paramTypeRequest) {
        ParamType paramType = paramTypeRepository.findById(id).orElseThrow(
                () -> new EntityNotFoundException(StringConstants.PARAM_TYPE_NOT_FOUND));
        paramTypeMapper.updateParamTypeRequestToParamType(paramType, paramTypeRequest);
    ...
    public void delete(Long id) {
        if (paramTypeRepository.findById(id).isEmpty()) {
            throw new EntityNotFoundException(StringConstants.PARAM_TYPE_NOT_FOUND);
        }
